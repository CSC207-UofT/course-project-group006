...
//        Port number: 3306
    ...
    public void testTestAnswer() {
        ...
        new QuestionManager(questionGateway).addQuestion("test2", "banana", "xiangjiao", 4);
        new TestManager(testGateway).creatTest("test", 1, 10);
        new TestManager(testGateway).addQuestionToTest(1, 1);
        new TestManager(testGateway).addQuestionToTest(1, 2);
        new QuestionAnswerManager(questionAnswerGateway).createQuestionAnswer(questionID, "pingguo", 1, 1, bb);
        new QuestionAnswerManager(questionAnswerGateway).createQuestionAnswer(questionID, "ping", 2, 1, bb);
        new QuestionAnswerManager(questionAnswerGateway).createQuestionAnswer(2, "xiangjiao", 1, 1, bb);
        new QuestionAnswerManager(questionAnswerGateway).createQuestionAnswer(2, "xiang", 2, 1, bb);
        new TestAnswerManager(testAnswerGateway, testGateway, questionAnswerGateway).createTestAnswer(1, 1, testAnswerGateway, testGateway, bb);
        new TestAnswerManager(testAnswerGateway, testGateway, questionAnswerGateway).createTestAnswer(1, 2, testAnswerGateway, testGateway, bb);
